...
with torch.no_grad():
    ...
    class NLMEModel:
        ...
        def log_likelihood_observation(
            ...
            #     ).item()
            # )  # each row of res_error_var is the variance of the residual error corresponding to the same row of residuals (one row = one output)
            # print(f"DEBUG likelihood {log_lik}")
            return log_lik
    ...
    class NLMEModel_from_GP(NLMEModel):
        ...
        def structural_model(
            ...
            required_cols = ["ind_id"] + required_params_cols + ["output_name"]
            ...
            unique_input_cols = ["ind_id"] + required_params_cols
            unique_inputs_df = (
                input_data[unique_input_cols].drop_duplicates().reset_index(drop=True)
            )
            input_cols = required_params_cols
    ...
    class NLMEModel_from_jinko(NLMEModel):
        """
        This class inherits from NLMEModel. A sid for a jinko model and associated folder_id must be given at initialization. The structural model will be constructed from it.
        Careful, if you use this class, you should have a .env file with your jinko API key, project id and base url.
        You should also have only one active arm in the protocol (else the first arm found will be used)
        """
        ...
        def structural_model(
            ...
            """
            Predicts outputs using calling the jinko API with inputs from the given DataFrame.
            This DataFrame must contain columns for all individual parameters and time points.
            input_data (DataFrame): DataFrame with columns corresponding to individual parameters, time, and output_name.
            Returns a DataFrame with the same columns plus an arm and a predicted_value column.
            """
            # ensure the input DataFrame has the necessary columns
            required_params_cols = self.MI_names + self.PDU_names
            required_cols = ["ind_id"] + required_params_cols + ["output_name"]
            ...
            # unique_time_steps = torch.unique(
            #     torch.round(torch.Tensor(input_data["time"].values) * 1e5) / 1e5,
            #     sorted=True,
            # )
            # get the unique parametercombinations
            # we need ind_id for merging later
            unique_input_cols = ["ind_id"] + required_params_cols
            unique_inputs_df = (
                input_data[unique_input_cols].drop_duplicates().reset_index(drop=True)
            )
            ...
            if unique_time_steps.numel() == 1:
                time_value = unique_time_steps[0].item()
                time_iso = pd.Timedelta.isoformat(
                    pd.to_timedelta(time_value, unit=self.time_steps_unit)
                )
                solving_times.append(
                    {"tMax": time_iso, "tMin": time_iso, "tStep": "PT0S"}
                )
            else:
                time_points_np = unique_time_steps.numpy()
                # give the first period that is the global period (encompasses all the others)
                solving_times.append(
                    {
                        "tMax": pd.Timedelta.isoformat(
                            pd.to_timedelta(
                                time_points_np[-1], unit=self.time_steps_unit
                            )
                        ),
                        "tMin": pd.Timedelta.isoformat(
                            pd.to_timedelta(
                                time_points_np[0], unit=self.time_steps_unit
                            )
                        ),
                        "tStep": pd.Timedelta.isoformat(
                            pd.to_timedelta(
                                (time_points_np[-1] - time_points_np[0]),
                                unit=self.time_steps_unit,
                            )
                        ),
                    }
                )
                # add the other timepoints. since jinko does not accept explicit times but only (start, end, step), we have to add a new solving_times period for each new different step
                start_time_value = time_points_np[0]
                current_step_value = time_points_np[1] - time_points_np[0]

                for i in range(1, time_points_np.size):
                    next_step_value = time_points_np[i] - time_points_np[i - 1]

                    if abs(next_step_value - current_step_value) > 1e-3:
                        end_time_value = time_points_np[i - 1]
                        solving_times.append(
                            {
                                "tMax": pd.Timedelta.isoformat(
                                    pd.to_timedelta(
                                        end_time_value, unit=self.time_steps_unit
                                    )
                                ),
                                "tMin": pd.Timedelta.isoformat(
                                    pd.to_timedelta(
                                        start_time_value, unit=self.time_steps_unit
                                    )
                                ),
                                "tStep": pd.Timedelta.isoformat(
                                    pd.to_timedelta(
                    ...
                    end_time_iso = pd.Timedelta.isoformat(
                        pd.to_timedelta(time_points_np[-1], unit=self.time_steps_unit)
                    )
                    start_time_iso = pd.Timedelta.isoformat(
                        pd.to_timedelta(start_time_value, unit=self.time_steps_unit)
                    )
                    current_step_iso = pd.Timedelta.isoformat(
                        pd.to_timedelta(current_step_value, unit=self.time_steps_unit)
                    )
                    solving_times.append(
                        {
                            "tMax": end_time_iso,
                ...
                vpop = {
                    "patients": [
                        self.to_patient(unique_inputs_df.iloc[i, :].values)
                        for i in range(unique_inputs_df.shape[0])
                    ]
                }
                response = jinko.make_request(
                ...
                if self.trial_core_item_id is None:  # create the trial the first time
                    response = jinko.make_request(
                ...
                timeseries_json = {
                    "timeseries": {
                        output: [self.arm_name] for output in self.outputs_names
                    }
                }
                try:
                    response = jinko.make_request(
                ...
                df_time_series = pd.read_csv(io.StringIO(csvTimeSeries)).rename(
                    columns={
                        "Patient Id": "ind_id",
                ...
                output_df = pd.merge(
                    input_data,
                    df_time_series,
                    on=["time"] + ["output_name"] + ["ind_id"],
                    how="left",
                )
                output_df["time"] = output_df["time"].round(
                    self.nb_decimal_rounding_time
                )
        ...
        def sample(
            ...
            if predictions_history:
                ...
                mean_predictions_df = all_predictions_df.groupby(
                    ["ind_id", "output_name"], as_index=False
                )["predicted_value"].mean()
            else:
                mean_predictions_df = DataFrame(
                    columns=["ind_id", "output_name", "predicted_value"]
                )
    ...
    class PySAEM_for_one_timepoint:
        """
